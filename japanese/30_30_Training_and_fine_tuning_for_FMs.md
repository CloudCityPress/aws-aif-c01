...
基盤モデルは、すべてのビジネス目的に対応できる状態で提供されるわけではありません。段階的に構築されており、各層が費用と野心に比例して特異性を高めていきます。ビジネスプロフェッショナルにとって、モデルがどのように構築・適応されるかを理解することは、主に技術的な演習ではなく、予算策定とスコーピングの演習です。事前トレーニング、ファインチューニング、継続的な事前トレーニング、および蒸留の違いを知ることで、エンジニアリングチームがカスタマイズプロジェクトを提案したときに適切な質問ができ、提案されたタイムラインと予算を評価でき、よりシンプルなアプローチで同等の結果が得られる場合を認識できます。[^303001]

このタスクステートメントの 3 つの目標は論理的な順序に従います。最初はトレーニング技術の分類とそのコストプロファイルをカバーします。2 番目はファインチューニングが適切な選択である場合に使用される具体的な手法をカバーします。3 番目は、ファインチューニングジョブが開始される前にデータをどのように準備しなければならないかをカバーします。これにはモデルの動作をビジネスの期待に合わせる人間のフィードバックプロセスも含まれます。これらが合わさることで、ビジネスのスポンサーがトレーニングコードを一行も書かずにモデルカスタマイズプロジェクトを委託・監督するために必要な枠組みが提供されます。

### 3.3.1 基盤モデルトレーニングの主要要素

基盤モデルのトレーニングは単一の活動ではありません。各段階が前の段階の上に構築される進行です。ビジネスが何を必要とし、どれだけ投資できるかに応じて、各段階をエントリーポイントとして利用できます。試験では 4 つの段階をカバーします。事前トレーニング、ファインチューニング、継続的な事前トレーニング、および蒸留です。これらはおよそコストの高い順から低い順、変化の範囲の広い順から狭い順に並んでいます。
...
ファインチューニングの**データセットサイズ**は、事前トレーニングからの直感が示すよりも小さいですが、些細なものでもありません。[^303022] タスク適応のための典型的なインストラクションチューニングジョブは、測定可能な改善を生み出すために数百から数千の高品質なラベル付き例を必要とします。品質対量の原則が直接適用されます。重複、誤り、トピック外のアイテムを含む 5,000 の例よりも、注意深くキュレートされた、正確で多様な 500 の例が一貫して優れています。意味のあるファインチューニングの実際的な最小値は通常 200 から 500 の例の範囲です。それ以下では、モデルが確実に汎化するのに十分な多様性に遭遇しません。上限では、狭く定義されたタスクの場合、タスクパフォーマンスの改善は通常数千の例後にプラトーに達し、その時点でデータを追加しても、新しい例が真に新しいサブシナリオをカバーしない限り、収穫逓減が生じます。
