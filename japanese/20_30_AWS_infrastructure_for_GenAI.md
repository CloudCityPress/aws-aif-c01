...
AWS 上で生成 AI アプリケーションを構築するには、開発の各段階に対応したマネージドサービスと開発者ツールの拡大するセットから適切なものを選ぶ必要があります。タスク 2.3 は 4 つの目標をカバーします。v1.1 試験ガイドで指定された AWS サービス、それらを使用する利点、それらが AWS から継承するセキュリティとコンプライアンスの特性、そして本番環境でチームが直面するコスト上の決定です。[^203001]
...
このタスクステートメントで取り上げるサービスは、単一の軸で競合する関係にはありません。チームは同じプロジェクト内で、**Amazon Bedrock** をモデル API として使用し、そのアプリケーションを **Amazon Bedrock AgentCore** を通じてデプロイし、**Kiro** の中で開発作業を自動化し、**Amazon Quick** を通じてビジネスデータをクエリすることもできます。以下のセクションでは、各サービスの概要、AWS プラットフォームを利用することの総合的なメリット、基盤となるセキュリティとコンプライアンスのインフラ、そして総所有コストを左右する料金の仕組みを説明します。
