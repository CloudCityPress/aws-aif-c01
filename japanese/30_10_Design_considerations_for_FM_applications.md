...
基盤モデル上で本番アプリケーションを構築するには、最初のプロンプトを書く前から多くの準備が必要です。設計段階での判断、使用するモデルの選択、出力の調整方法、プライベートデータによるグラウンディング、データの格納先、知識のスケール方法、これらすべてが、プロジェクトが価値を生み出すかパイロット段階で停滞するかを左右します。このタスクステートメントでは、ビジネスアーキテクトが直面する順序でそれらの決定を解説します。[^301001]

### 3.1.1 FM を選択するための選択基準

基盤モデルの選択は、一度きりの技術的判断ではありません。ビジネス要件が変化するたびに、その都度見直すことになります。パイロットで許容可能だったモデルが、本番ボリュームになると高すぎることがあります。英語の顧客質問に良く答えたモデルが、製品がスペイン語市場に拡大するときに替える必要が生じることがあります。選択基準を理解しておくことで、場当たり的ではなく体系的に判断を下せるようになります。

試験でカバーされる選択基準は、3 つのグループに整理されます。コストとパフォーマンスの基準はモデルの実行コストと応答速度に関わります。能力の基準はモデルが何をできるかを決めます。柔軟性の基準はモデルをビジネスニーズに合わせてどこまで変えられるかを左右します。
...
**モデルの複雑さ**はパラメータ数を超えたアーキテクチャ上の設計選択に関係します。一部のモデルは密（dense）で、すべてのトークンについてすべてのパラメータが活性化します。その他は*混合エキスパート（MoE）*アーキテクチャを使用し、トークンあたりパラメータのサブセットのみを活性化して、低い推論コストでより良い品質を達成します。[^301039] モデル選択の観点では、アーキテクチャの複雑さは推論スループットと必要なインフラ規模に直接影響するため、無視できない要素です。
...
正しく選択されたモデルでも、推論パラメータが誤って設定されると不適切な出力を生成することがあります。推論パラメータは、プロンプトと同時に実行時に渡される設定値で、次トークンの確率分布からどのようにサンプリングするかをモデルに指示します。これらを調整すれば、モデルを再トレーニングしなくても出力の傾向を変えられます。
...
基盤モデルは大規模な公開データセットでトレーニングされていますが、トレーニングのカットオフ以降の情報にはアクセスできず、独自の組織データにもアクセスできません。2024 年後半までトレーニングされたモデルは 2025 年初頭の製品リリースについての質問に答えられません。汎用モデルは社内の HR ポリシー、顧客契約テンプレート、エンジニアリングランブックを見たことがありません。**検索拡張生成（RAG）**は、トレーニングを通じてモデルの重みに知識を組み込むのではなく、クエリ時にモデルを外部知識ストアに接続することで、この制限に対処するアーキテクチャパターンです。[^301008]

RAG は 5 つのステップで動作します。まず、ユーザーの質問が意味を数値化した*エンベディング*ベクトルに変換されます。[^301047] 次に、そのベクトルを、組織のプライベートドキュメントから事前に計算したエンベディングのデータベースと照合します。続いて、クエリのエンベディングに最も類似するドキュメントを取得します。取得したドキュメントはコンテキストブロックとしてまとめられ、ユーザーの元の質問の前に付加されて完全なプロンプトが形成されます。最後に、基盤モデルがこの拡張プロンプトを読み、モデル内部の知識だけでなく取得したコンテンツにも基づいてレスポンスを生成します。[^301009]
...
**Amazon Neptune Analytics** は Neptune グラフデータベースをベクトル検索機能で拡張し、グラフトラバーサルと意味的類似性を組み合わせたクエリを可能にします。[^301015] 人、組織、ドキュメント間の関係をモデル化した知識グラフは、Neptune Analytics を使って「法務部門の誰かが書いて少なくとも 1 つの規制を引用している、このクエリに最も意味的に類似したドキュメントを見つけてください」というような質問に答えることができます。[^301057] グラフ推論とベクトル検索を組み合わせたこのアプローチは、リレーショナルデータベースや通常の検索ベースのストアでは実現が難しい問い合わせを可能にします。Neptune Analytics は、サプライチェーン分析、不正調査、生物医学研究など、検索問題が固有のグラフ構造を持つ場合に適切な選択です。
...
**AI エージェント**は、基盤モデルに環境認識、アクション計画、外部ツールの実行、結果の観察、そして学習内容に基づく計画修正の能力を組み合わせたシステムです。[^301024] エージェント内のモデルは単にテキストを生成しているわけではありません。次に何をすべきかを推論し、どのツールを呼び出すかを決定し、結果が十分かを評価し、タスクが完了するか停止条件に達するまで、このサイクルを繰り返します。[^301066]
