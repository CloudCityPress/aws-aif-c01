## Формулировка задачи 1.1: Объяснить основные концепции и терминологию ИИ

Словарь ИИ — это общий язык между бизнес-профессионалами и инженерными командами, с которыми они работают. Прежде чем продакт-менеджер сможет утвердить развёртывание модели, а руководитель — оценить коммерческое предложение вендора ИИ, все участники должны иметь одинаковые определения таких терминов, как обучение, инференс, предвзятость и справедливость. Данная формулировка задачи устанавливает этот общий словарь и отображает каждый термин на сервисы AWS и цели экзамена, где он встречается.[^101001]
...
**Машинное обучение** — это подмножество ИИ, ограничивающее определение системами, которые учатся на данных. Фильтр мошенничества на основе правил, написанный программистом, является ИИ, но не МО. Модель обнаружения мошенничества, обученная на истории транзакций, является и ИИ, и МО.

**Глубокое обучение** — это подмножество МО, в котором используются многослойные нейронные сети. Модель линейной регрессии является МО, но не глубоким обучением. Свёрточная нейронная сеть, классифицирующая рентгеновские снимки грудной клетки, является МО, глубоким обучением и ИИ одновременно.

**Генеративный ИИ** — это подмножество глубокого обучения, специально занимающееся генерацией нового контента. Не всё глубокое обучение является генеративным: модель глубокого обучения, классифицирующая изображения по десяти категориям, является дискриминативной, а не генеративной. Модель, производящая фотореалистичное изображение из текстового описания, является генеративным ИИ.

**Агентный ИИ** — это архитектурный паттерн, наложенный поверх генеративного ИИ. Агентная система использует LLM или другую генеративную модель в качестве механизма рассуждения, затем добавляет петлю планирования, доступ к инструментам и память, чтобы действовать в несколько шагов. Одноходовой чат-бот, использующий LLM, является генеративным ИИ, но не агентным ИИ. Система, получающая высокоуровневую цель, разбивающая её на подзадачи, использующая инструменты для выполнения каждой подзадачи и синтезирующая результаты, является агентным ИИ.
...
flowchart TD
 ...
 F --> G[Агентный ИИ]
```
*Рисунок 1.1.1: Вложенность подобластей ИИ. Каждый узел является правильным подмножеством своего родителя; движение вниз по дереву добавляет ограничения и возможности, а не заменяет родительскую концепцию.*
...
Стоит отметить один нюанс: некоторые исследователи классифицируют агентный ИИ как архитектурный паттерн, а не технологическую подобласть, поскольку агентная система состоит из существующих технологий (LLM, инструменты, логика оркестрации), а не является новым типом модели. Для целей экзамена рассматривайте агентный ИИ как наиболее специализированный уровень в иерархии.
...
```
*Рисунок 1.1.3: Дерево решений по типу данных. Наличие структуры и меток вместе определяют, какой подход к обучению реализуем для данного набора данных.*
...
Размеченные данные определяются наличием правильного вывода, сопровождающего каждый входной пример.[^101049] В данном сценарии исторические записи включают переменную результата (ушедший или удержанный), которую модель с учителем научится предсказывать. Формат данных (в данном случае табличный) является отдельным измерением от разграничения «размеченные-неразмеченные». Запись может быть одновременно табличной и размеченной.
